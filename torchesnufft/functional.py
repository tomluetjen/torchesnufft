...
def get_density(x, c, N, eps=1e-6, n_iter=10):
    ...
    # n_iter = 1 corresponds to Jackson JI, Meyer CH, Nishimura DG,  Macovski A. Selection of a convolution function for Fourier inversion using gridding (computerised tomography application). IEEE Transactions on Medical Imaging. 1991 Sept;10(3):473-78. doi: 10.1109/42.97598.
    d = len(N)
    device = c.device
    ...
    # Normalize so the mean density is 1, preserving the pure-adjoint 1 / M scaling.
    return c.shape[-1] * density / density.sum(dim=-1, keepdim=True)
